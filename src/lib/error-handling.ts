...
export const retry = async <T>(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (isAuthenticationError(error) || isAuthorizationError(error) || isNotFoundError(error)) {
        throw error
      }
...
export const formatErrorBoundaryError = (error: Error, errorInfo: { componentStack: string }): AppError => {
  return new AppError(
...
export const parseApiError = (response: Response, data: unknown): ApiError => {
