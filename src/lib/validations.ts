...
export const createCourseSchema = z.object({
  ...
  courseId: z
    ...
    .min(3, 'Course ID must be at least 3 characters')
    .max(50, 'Course ID must be less than 50 characters')
    .regex(/^[A-Z0-9\-_]+$/, 'Course ID must contain only uppercase letters, numbers, hyphens, and underscores'),
  certificateTemplate: z
    ...
    }, 'Certificate template must be a PDF or image file')
})
...
export const uploadLearnersSchema = z.object({
  file: z
    .instanceof(File)
    .refine((file) => file.type === 'text/csv', 'File must be a CSV file')
...
export type UploadLearnersInput = z.infer<typeof uploadLearnersSchema>
...
export const getFormErrors = (errors: z.ZodError | undefined): Record<string, string> => {
  if (!errors) return {}
